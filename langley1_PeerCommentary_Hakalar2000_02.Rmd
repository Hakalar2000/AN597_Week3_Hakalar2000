...
* The average number of successes (μ) that occurs in a specified region is known. We believe the average is 15 calls in a 2 hour period. 
          LA: Wouldn't the average number of successes in a poisson dist. be λ not μ ?
...
#LA: My understanding is that if q=8, thats just the probability right at 8, since you haven't noted a range of values (i.e. 0:8 or 8:20) but I may be wrong, especially if changing the lower.tail function changes those parameters as you noted above. 
#LA: What I did, is it tried to find the probability for 0:8 and then take the inverse of that for the probability of 'more than 8' but I'm not fully confident that what I did is right (I did get a similar probability value to you though)
```
...
#LA: I think the "space = 0" is making your bars touch with no space in between them. If you change it to "space = 1", then the bars move apart from one another. The "names.arg" function is telling R to plot the values associated with variable "x" below the bars on the x axis, which is why we see 0-30. 
```
...
#LA: I did the same thing! I know we should be able to use boxplotting but I could not figure it out. 
```
...
#LA: How would you say your histogram compares to the shape of the probability mass function plotted up above?
```
...
1. It makes sense when you say you use ppois() for a variable that is not discrete whereas dpois() is for discrete variables (i.e. 8). I had difficulty understanding when to use ppois() vs dpois() so that is helpful. 
2. When you say line graph, do you mean bar graph? I think what you have plotted up above is a technically a bar graph.
3. Look throughout code for my other comments! (noted by #LA:)
4. Good job!
